   ...
   $(document).ready(function(){  // onload 이벤트
      $('#findPwBtn').click(function(){  // click 이벤트
         ...
         $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            data: 'sId=' + $('#sId').val() + '&sEmail=' + $('#sEmail').val(),
      ...
      $('#realFindPwBtn').click(function(){
         ...
         if ($('#authKey').val() == authKey && authKey != "") {
            alert('인증되었습니다.');
            location.href = 'changeSellerPwPage?sId=' + $('#sId').val();
            }else if(mId == ""){
            alert('이메일인증을 진행하세요.');
         }
         else {
...
<div class="find-pw-box">
   ...
   <form method="POST">
       ...
       <table>
         <tr>
            <td>아이디</td>
            <td><input id="sId" type="text" name="sId" autofocus /></td>
         </tr>
         <tr>
            <td>이메일</td>
            <td>
               <input id="sEmail" type="text" name="sEmail"/>
