   ...
   $(document).ready(function(){  // onload 이벤트
      $('#changePwBtn').click(function(){  // click 이벤트
         ...
         $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            data: 'sId=' + $('#sId').val() + '&sPw=' + $('#sPw').val(),
            success: function( data ) {
               if ( data.result == 'SUCCESS' ) {
                  alert('새로운 비밀번호로 변경되었습니다.');
                  location.href = 'sellerMain';
               } else {
   ...
   <form id="f" method="POST">
      ...
      <table>
         <tbody>
            <tr>
               <td>아이디</td>
               <td><input id="sId" type="text" name="sId" value="${param.sId }" readonly /></td>
            </tr>
            <tr>
               <td>새로운 비밀번호</td>
               <td><input id="sPw" type="password" name="sPw" autofocus /></td>
            </tr>
            <tr>
               <td>비밀번호 확인</td>
               <td><input id="sPw2" type="password" name="sPw2" /></td>
            </tr>
         </tbody>
      </table>
      <div class="btnbox">
