...
<script type="text/javascript">
   ...
   $(document).ready(function(){
      ...
      $('#sId').keyup(function(){
         $.ajax({
            ...
            success: function( responseObject ) {
               // 정규식 test()
               if ( regExpID.test($('#sId').val()) ) {  // 정규식을 만족하면,
                  // 가입된 아이디인지 여부 확인
                  ...
                  $('#idCheckResult').text('10자리의 사업자 번호를 입력해주세요.');
                  $('#idCheckResult').css('color', 'red');
                  idFlag = false;
               }
            },
      ...
      $('#sEmail').keyup(function() {
         $.ajax({
            ...
            success: function( responseObject ) {
               // 정규식 test()
               if ( regExpEMAIL.test($('#sEmail').val()) ) {
                  // 가입된 이메일 여부 확인
                  if ( responseObject.result == 'NO' ) {
      ...
      // 4. 전화번호(핸드폰)
         ...
         $('#sPhone').keyup(function(){
            $.ajax({
               ...
               success: function( responseObject ) {
                  // 정규식 test()
                  if ( regExpPHONE.test($('#sPhone').val()) ) {  // 정규식을 만족하면,
                     // 가입된 아이디인지 여부 확인
                     if ( responseObject.result == 'NO' ) {
      ...
      $('#signUpBtn').click(function(){
         ...
         if(idFlag && pwFlag && pw2Flag && nameFlag && authFlag && phoneFlag){
            if ($('#authKey').val() == authKey && authKey != "") {
               register.action='sellerregister';
               register.submit();
            }else {
   ...
   //사업자번호인증api
    function businessNumber(){ //사업자번호 인증 API 이용
        let num = document.getElementById('sId').value; //사업자번호
        const data = {
            ...
            <div class="register-box">
               <table id="register-table">
                  ...
                  <!-- 사업자번호 확인 api 가져오기 -->
                     <td>사업자번호</td>
                     <td>
                        <input id="sId" type="text" name="sId" style="width:250px;" autocomplete="off" placeholder="ex)0000000000" />
                        <input type="button" value="사업자등록번호조회"  onclick="businessNumber()"/>
                        <br/>
                  ...
                  <tr>
                     <td>비밀번호</td>
                     <td>
                        <input id="sPw" type="password" name="sPw" placeholder="비밀번호를 입력해주세요" /><br/>
                  ...
                  <tr>
                     <td>비밀번호 확인</td>
                     <td>
                        <input id="sPw2" type="password" name="sPw2" placeholder="비밀번호를 확인해주세요"  /><br/>
                  ...
                  <tr>
                     <td>이메일</td>
                     <td>
                        <input id="sEmail" type="text" name="sEmail" style="width:250px;" placeholder="이메일을 입력해주세요"/> 
                  ...
                  <tr>
                     <td>전화번호</td>
                     <td>
                        <input id="sPhone" type="text" name="sPhone" placeholder="ex)01000000000" /><br/>
